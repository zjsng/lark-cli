---
title: "List Subtask"
url: "https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/task-v2/task-subtask/list"
method: "GET"
...
Paging is supported, and data is returned in the order in which subtasks are placed on the Lark App UI.

> Read permission of the parent task is required. Please refer to the "How are Tasks Authorized?" section in Task Features Overview.
...
| --- | --- | --- |
| `task_guid` | `string` | GUID of parent task **Example value**: "d300a75f-c56a-4be9-80d1-e47653028ceb" **Data validation rules**: - Maximum length: `100` characters | ### Query parameters
| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
...
|         `url` | `string` | The url corresponding to the task. |
|         `title` | `string` | The title corresponding to the task |
...
| --- | --- | --- | --- |
| 400 | 1470400 | The request parameter is an error, such as the parent task GUID is invalid, or the page_size is set to a negative number. | The reason for the error is shown in the returned msg prompt. |
| 404 | 1470404 | The parent task does not exist or has been deleted. | Confirm that the parent task you want to access exists or has been deleted. |
| 500 | 1470500 | Server error. | Try retrying the call. If it continues to fail, please contact support for feedback. |
| 403 | 1470403 | Missing read permission for the parent task. | Check whether the calling identity has the read permission of the parent task. Please refer to the "How are Tasks Authorized?" section in Task Features Overview. |
