---
title: "Add chat menu"
url: "https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/chat-menu_tree/create"
method: "POST"
...
This interface is used to add a chat menu to a chat.

> Notice:
> - This interface is to add menus to the group, and the existing menus in the group will not be overwritten.
> - After the interface is called successfully, all chat menus in the current chat will be returned.
> - The bot ability needs to be enabled.
> - The bot must be in the chat.
> - In a chat, there are at most 3 top level menus, and each top level menu has at most 5 second level menus.
> - Adding a second_level menu to the top_level menu is not currently supported.
> - - Only chats whose chat mode is group are supported.
...
| --- | --- | --- |
| `chat_id` | `string` | Chat ID. For details, refer to Group ID description. **Note**: Only supports group IDs whose group mode is `group`. **Example value**: "oc_a0553eda9014c201e6969b478895c230" | ### Request body

| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
| `menu_tree` | `chat.menu_tree` | Yes | The menu to add to the chat. |
|   `chat_menu_top_levels` | `chat_menu_top_level[]` | Yes | Top level menu |
|     `chat_menu_item` | `chat_menu_item` | Yes | Top level menu info |
|       `action_type` | `string` | Yes | Menu type **Notice** - If the top_level menu has a second_level menu, the value of this top_level menu must be NONE. **Example value**: "NONE" **Optional values are**:  - `NONE`: NONE type, when only the top level menu has a second level menu, the top level menu is set to NONE type. - `REDIRECT_LINK`: REDIRECT_LINK type  |
...
|       `name` | `string` | Yes | Menu name. **Notice** - The number of characters in the name must be in the range of 1 to 120. **Example value**: "group chat" |
...
|         `ja_jp` | `string` | No | Japanese name **Example value**: "サインアップ" |
|     `children` | `chat_menu_second_level[]` | No | Top level menu |
|       `chat_menu_item` | `chat_menu_item` | No | Second level menu |
...
| `data` | `\-` | \- |
|   `menu_tree` | `chat.menu_tree` | The existing menus in the chat after the addition |
...
| 400 | 232070 | The number of chat_menu_top_level has reached the limit. | The number of chat_menu_top_level has reached the limit. |
| 400 | 232071 | The number of chat_menu_second_level has reached the limit. | The number of chat_menu_second_level has reached the limit. |
