---
title: "Sort chat menus"
url: "https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/chat-menu_tree/sort"
method: "POST"
api_path: "https://open.larksuite.com/open-apis/im/v1/chats/:chat_id/menu_tree/sort"
service: "im-v1"
...
Sort the top level menu in a chat.

> Notice:
> - The bot ability needs to be enabled.
> - The bot must be in the chat.
> - After operating the API, all menus in the chat will be returned.
...
| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
| `chat_menu_top_level_ids` | `string[]` | Yes | For the order of the new top level menu, the sorting ID list needs to be consistent with the current top level menu ID list in the chat. Get the menus through the Get Chat Menu. **Example value**: ["6936075528890826780"] | ### Request body example
...
| `data` | `\-` | \- |
|   `menu_tree` | `chat.menu_tree` | Chat menu after sorting. |
...
| 400 | 232072 | Sort chat_menu_top_level's param is invalid. | The incoming top level menu ID list is inconsistent with the current top level menu ID list in the chat. |
