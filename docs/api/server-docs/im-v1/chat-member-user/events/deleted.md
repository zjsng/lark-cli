---
title: "Users leave a group"
url: "https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/chat-member-user/events/deleted"
service: "im-v1"
...
The event is pushed when users leaves a group voluntarily or are removed from a group chat.{Usage Examples}(url=/api/tools/api_explore/api_explore_config?project=im&version=v1&resource=chat.member.user&event=deleted)

> Notes:
> - The bot ability needs to be enabled, and the above changes occur in the group that the bots belong to.
> - The bot needs to subscribe to the Users leave a group voluntarily or are removed from a group chat event under the Messenger category.
> - The event is pushed to bots who have subscribed to this event in the group.

## Event
| Facts |  |
| --- | --- |
| Event type | im.chat.member.user.deleted_v1 |
...
|   `operator_tenant_key` | `string` | The operator's tenant, which is the unique identifier of the tenant on Lark, which is used to exchange for the corresponding tenant_access_token, and can also be used as the unique identifier of the tenant in the application |
|   `users` | `chat.member.user[]` | List of users removed from a group chat |
