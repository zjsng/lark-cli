...
url: "https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/message-reaction/events/deleted"
service: "im-v1"
...
This event is triggered when a reaction for a message is deleted.{Usage Examples}(url=/api/tools/api_explore/api_explore_config?project=im&version=v1&resource=message.reaction&event=deleted)

> Notes:
> - Bot ability must be enabled.
> - To receive the pushed messages, you must have the scope Read messages in private and group chats or Read and send messages in private and group chats and subscribe to reaction.deleted events under the Messenger category.
> - A bot can only receive reaction deletion events for messages in the group chat where the bot resides.

## Event
| Facts |  |
| --- | --- |
| Event type | im.message.reaction.deleted_v1 |
...
| `event` | `\-` | \- |
|   `message_id` | `string` | The open_message_id of the message |
|   `reaction_type` | `emoji` | Reaction type |
|     `emoji_type` | `string` | Emoji type. For more information, see Emoji type enumeration. |
|   `operator_type` | `string` | Operator type Notes: - If the operator type is "user", user_id will be returned - If the operator type is "app", app_id will be returned |
...
|   `app_id` | `string` | App ID |
|   `action_time` | `string` | Timestamp that indicates when the reaction was added, in ms | ### Event body example
