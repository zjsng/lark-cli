---
title: "Update app feed card"
url: "https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/group/im-v2/app_feed_card-batch/update"
method: "PUT"
...
updated: "1717121394000"
---

# Update app feed card

This interface is used to update the avatar, title, preview, label status, button and other information of the information flow card.

> **Field Update Strategy**: The specific fields to be updated are based on `update_fields`. The fields represented by `update_fields` are extracted from `app_feed_card` for updates. Fields not included in `update_fields` will not be updated.
...
| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
| `feed_cards` | `user_open_app_feed_card_updater[]` | No | feed cards **Data validation rules**: - Length range: `1` ～ `20` |
|   `app_feed_card` | `open_app_feed_card` | Yes | app feed card |
...
|     `avatar_key` | `string` | No | avatar key **Example value**: "v3_0041_007bca9f-67ba-4199-bf00-4031b12cf226" **Data validation rules**: - Length range: `0` ～ `100` characters |
|     `preview` | `string` | No | preview **Example value**: "preview text" **Data validation rules**: - Length range: `0` ～ `120` characters |
|     `status_label` | `open_feed_status_label` | No | status label |
|       `text` | `string` | Yes | label text **Example value**: "label text" **Data validation rules**: - Length range: `1` ～ `20` characters |
...
|   `user_id` | `string` | Yes | user id **Example value**: "ou_88553eda9014c201e6969b478895c223" |
|   `update_fields` | `string[]` | Yes | update fields **Example value**: ["1"] **Optional values are**:  - `1`: title - `2`: avatar key - `3`: preview - `10`: status label - `11`: buttons - `12`: link - `13`: time sensitive - `101`: update display time to current - `102`: update rank time to current - `103`: notify setting  **Data validation rules**: - Length range: `1` ～ `20` | ### Request body example
...
|     `reason` | `string` | reason **Optional values are**:  - `0`: unknown - `1`: no permission - `2`: not created - `3`: rate limited - `4`: duplicated  | ### Response body example
