---
title: "File deleted to trash can"
url: "https://open.larksuite.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/event/delete-file-to-trash-can"
section: "Docs"
scopes:
  - "drive:drive"
  - "docs:doc"
...
updated: "1768985219000"
---

# File deleted to trash can

This event is triggered when a subscribed file is deleted to the recycle bin. 
...
| Facts |  |
| --- | --- |
| Event type | drive.file.trashed_v1 |
| Supported app types | custom,isv |
| Required scopes Enable any scope from the list | `drive:drive` `docs:doc` `sheets:spreadsheet` `docs:event.document_deleted:read` |
...
| --- | --- | --- |
| `schema` | `string` | Version of the event schema. Absent in version 1.0. |
| `header` | `event_header` | Header of the event. |
|   `event_id` | `string` | Event ID. |
|   `event_type` | `string` | Type of the event. |
|   `token` | `string` | Event token. |
|   `create_time` | `string` | Timestamp when the event was created (milliseconds). |
|   `tenant_key` | `string` | Tenant key, representing the enterprise identifier. |
|   `app_id` | `string` | Application ID. |
|   `resource_id` | `string` | Cloud document token subscribed to. |
|   `user_list` | `string` | List of subscribed users. |
|     `union_id` | `string` | Union ID of the user. |
| `event` | `\-` | \- |
|   `file_type` | `string` | Type of the cloud document, supporting the following enumerations: - doc: Old version document. Deprecated. - docx: New version document. - sheet: Spreadsheet. - bitable: Multi-dimensional table. - slides:Slides - file:File |
|   `file_token` | `string` | Cloud document token. |
|   `operator_id` | `-` | Operator's ID. |
|     `union_id` | `string` | Union ID of the user. |
|     `user_id` | `string` | User's ID. **Field Permission Requirements**: `contact:user.employee_id:readonly` |
|     `open_id` | `string` | User's Open ID. | ## Event body example

```json
{
	"schema": "2.0",  // Version of the event schema. Absent in version 1.0.
	"header": {
		"event_id": "88ba7a69073dcc5b0dea70be77xxxxxx",   // Unique identifier of the event
		"token": "xxxxxx",  // Event token
		"create_time": "1612246959000",  // Timestamp when the event was created (milliseconds)
		"event_type": "drive.file.trashed_v1",  // Type of the event
		"tenant_key": "xxxxxx", // Tenant key, representing the enterprise identifier
		"app_id": "cli_xxxxxx",  // Application ID
		"resource_id": "doccnfYZzTlvXqZIGTdAHKabcef",  // Cloud document token subscribed to
	...
	"event": {
		...
		"file_type": "docx",	// Type of the file, supporting doc, sheet, docx, bitable
		"operator_id": { // Operator's ID
			"open_id": "ou_xxxxxx",
			"union_id": "on_xxxxxx",
			"user_id": "xxxxxx"
