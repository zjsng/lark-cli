---
title: "File read"
url: "https://open.larksuite.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/event/file-read"
section: "Docs"
scopes:
  - "drive:drive"
  - "docs:doc"
...
updated: "1768985087000"
---

# File read

File read event. This event is triggered when a cloud document is opened. 
...
| Facts |  |
| --- | --- |
| Event type | drive.file.read_v1 |
| Supported app types | custom,isv |
| Required scopes Enable any scope from the list | `drive:drive` `docs:doc` `sheets:spreadsheet` `docs:event.document_opened:read` |
...
| --- | --- | --- |
| `schema` | `string` | Version of the event format. If this field is absent, the version is 1.0. |
...
| `event` | `-` | - |
|   `file_type` | `string` | Cloud document type, supporting the following enumerations: - `doc`: Old version document, not recommended for use - `docx`: New version document - `sheet`: Spreadsheet - `bitable`: Multi-dimensional table - `slides`:Slides |
|   `file_token` | `string` | Multi-dimensional table token |
|   `operator_id` | `-` | Operator ID |
|     `union_id` | `string` | User's Union ID |
|     `user_id` | `string` | User's User ID **Field permission requirements**: `contact:user.employee_id:readonly` |
...
```json
{
  ...
  "header": {
    ...
    "event_type": "drive.file.read_v1", // Event type
  ...
  "event": {
    ...
    "file_type": "docx", // File type, supporting doc and sheet
    "operator_id_list": [
      { // Operator ID list
        "open_id": "ou_xxxxxx",
        "union_id": "on_xxxxxx",
