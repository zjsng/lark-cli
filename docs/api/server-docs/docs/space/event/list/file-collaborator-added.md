---
title: "File collaborator added"
url: "https://open.larksuite.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/event/file-collaborator-add"
section: "Docs"
scopes:
  - "drive:drive"
  - "docs:doc"
...
updated: "1768985180000"
---

# Add a file collaborator
This event is triggered when a user or group is added as a collaborator to a file.
...
| Facts |  |
| --- | --- |
| Event type | drive.file.permission_member_added_v1 |
| Supported app types | custom,isv |
| Required scopes Enable any scope from the list | `drive:drive` `docs:doc` `sheets:spreadsheet` `docs:permission.member:retrieve` |
...
| --- | --- | --- |
| `schema` | `string` | Event schema version. Absent for version 1.0. |
| `header` | `event_header` | Event header |
|   `event_id` | `string` | Event ID |
|   `event_type` | `string` | Event type |
|   `token` | `string` | Event token |
|   `create_time` | `string` | Event creation timestamp (milliseconds) |
|   `tenant_key` | `string` | Tenant key, representing enterprise identifier |
|   `app_id` | `string` | Application ID |
|   `resource_id` | `string` | Subscribed cloud document token |
|   `user_list` | `list` | Subscribed user list |
|     `union_id` | `string` | User's Union ID |
| `event` | `\-` | \- |
|   `chat_list` | `list` | List of added group IDs |
|   `file_type` | `string` | Cloud document type, supporting the following enums: - doc: Legacy document (deprecated) - docx: Modern document - sheet: Spreadsheet - bitable: Multi-dimensional table - slides:Slides - file:File |
|   `file_token` | `string` | Cloud document token |
|   `operator_id` | `-` | Operator ID |
|     `union_id` | `string` | User's Union ID |
|     `user_id` | `string` | User's User ID **Field Permission Requirement**: `contact:user.employee_id:readonly` |
|     `open_id` | `string` | User's Open ID |
|   `user_list` | `list` | List of added user IDs |
|     `union_id` | `string` | User's Union ID |
|     `user_id` | `string` | User's User ID **Field Permission Requirement**: `contact:user.employee_id:readonly` |
...
```json
{
  "schema": "2.0", // Event schema version. If this field is absent, it defaults to version 1.0
  "header": {
    "event_id": "88ba7a69073dcc5b0dea70be77xxxxxx", // Unique identifier for the event
    "token": "xxxxxx", // Event token
    "create_time": "1612246959000", // Event creation timestamp (milliseconds)
    "event_type": "drive.file.permission_member_added_v1", // Event type
  ...
  "event": {
    "chat_list": [
      "oc_xxxxxx"
    ], // List of added group IDs
    "file_token": "doccnfYZzTlvXqZIGTdAHKabcef", // File token
    "file_type": "doc", // File type, supports doc, sheet, docx, bitable
    "operator_id": { // List of operator IDs
      "open_id": "ou_xxxxxx",
      "union_id": "on_xxxxxx",
      "user_id": "xxxxxx"
    },
    "user_list": [
      { // List of added user IDs
        "open_id": "ou_xxxxxx",
        "union_id": "on_xxxxxx",
        "user_id": "xxxxxx"
      }
