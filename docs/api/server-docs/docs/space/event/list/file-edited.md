---
title: "File edited"
url: "https://open.larksuite.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/event/file-edited"
section: "Docs"
scopes:
  - "drive:drive"
  - "docs:doc"
...
updated: "1768985121000"
---

# File edited

This event is triggered when a file is edited, including base app record or field changes. Subscribing to this event allows for notifications when cloud documents are edited. 
...
| --- | --- | --- |
| `schema` | `string` | Version of the event format. Absent in version 1.0. |
...
|   `tenant_key` | `string` | Tenant key, representing the enterprise identifier. |
|   `app_id` | `string` | Application ID. |
|     `union_id` | `string` | User's Union ID. |
| `event` | `\-` | \- |
|   `file_type` | `string` | Cloud document type, supporting the following enumerations: - `doc`: Old version document, not recommended for use - `docx`: New version document - `sheet`: Spreadsheet - `bitable`: Base - `slides`:Slides |
|   `file_token` | `string` | Cloud document token. |
|   `operator_id_list` | `array` | List of operator IDs. |
|     `union_id` | `string` | User's Union ID. |
|     `user_id` | `string` | User's User ID **Field Access Requirements**: `contact:user.employee_id:readonly` |
|     `open_id` | `string` | User's Open ID. |
|   `subscriber_id_list` | `array` | List of subscribed user IDs. |
|     `union_id` | `string` | User's Union ID. |
|     `user_id` | `string` | User's User ID **Field Access Requirements**: `contact:user.employee_id:readonly` |
...
```json
{
  "schema": "2.0", // Event format version. Absent in version 1.0.
  "header": {
    "event_id": "88ba7a69073dcc5b0dea70be77xxxxxx", // Unique identifier for the event.
    "token": "xxxxxx", // Event token.
    "create_time": "1612246959000", // Event creation timestamp (in milliseconds).
    "event_type": "drive.file.edit_v1", // Type of the event.
    "tenant_key": "xxxxxx", // Tenant key, representing the enterprise identifier.
    "app_id": "cli_xxxxxx" // Application ID.
  },
  "event": {
    "file_token": "doccnfYZzTlvXqZIGTdAHKabcef", // Token of the edited file.
    "file_type": "docx", // Type of the file edited, supports doc, sheet.
    "operator_id_list": [ // List of editor IDs.
      {
        "open_id": "ou_xxxxxxxxxxxxxxxxxxxxxxxxx", // Editor's Open ID.
        "union_id": "on_xxxxxxxxxxxxxxxxxxxxxxxxx", // Editor's Union ID.
        "user_id": "xxxxxxxx" // Editor's User ID.
      }
    ],
    "subscriber_id_list": [ // List of subscriber IDs.
      {
        "open_id": "ou_xxxxxxxxxxxxxxxxxxxxxxxxx", // Subscriber's Open ID.
        "union_id": "on_xxxxxxxxxxxxxxxxxxxxxxxxx", // Subscriber's Union ID.
        "user_id": "xxxxxxxx" // Subscriber's User ID.
      }
    ]
  }
}
