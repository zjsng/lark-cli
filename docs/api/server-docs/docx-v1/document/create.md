...
url: "https://open.larksuite.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document/create"
method: "POST"
api_path: "https://open.larksuite.com/open-apis/docx/v1/documents"
service: "docx-v1"
resource: "document"
section: "Docs"
...
> **Application frequency limit:** the maximum frequency of a single application call is 3 times per second, beyond which the interface will return HTTP status code 400 and error code 99991400. When a request is frequency-limited, the application needs to handle the frequency-limited status code and use an exponential fallback algorithm or some other frequency-control strategy to reduce the rate of calls to the API.
...
| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
| `folder_token` | `string` | No | Folder Token, get the way to refer to How to get the token of docs resources, do not pass or pass empty means the root directory. **Example value**: "fldcnqquW1svRIYVT2Np6IuLCKd" |
| `title` | `string` | No | Document title **Example value**: "undefined" **Data validation rules**: - Length range: `1` ～ `800` characters | ### Request body example
...
| `data` | `\-` | \- |
|   `document` | `document` | Document information for new documents |
...
| 400 | 1770039 | folder not found | The current folder does not exist. Check whether Folder Token is filled in correctly |
| 403 | 1770040 | no folder permission | No permission to create documents in this folder |
...
| 400 | 1770036 | folder locked | Concurrent creation of documents in the same folder is not supported, please call this api serially in the same folder |
| 400 | 1770037 | folder size exceeded limit | The number of documents in the folder exceeds the limit |
