---
title: "Batch update blocks in group announcement"
url: "https://open.larksuite.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/chat-announcement-block/batch_update"
method: "PATCH"
api_path: "https://open.larksuite.com/open-apis/docx/v1/chats/:chat_id/announcement/blocks/batch_update"
service: "docx-v1"
...
updated: "1739935027000"
---

# Batch update blocks in group announcement

Batch update blocks and return their updated rich text content.
...
| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
| `requests` | `update_block_request[]` | No | Batch update blocks. Multiple updates to the same block in a batch update are not supported. That is, the Block ID in the request body cannot be repeated. **Data validation rules**: - Maximum length: `200` |
|   `update_text_elements` | `update_text_elements_request` | No | Update text element request |
|     `elements` | `text_element[]` | Yes | Updated text element list. For a single update: - The limit for `reminder` is 30 - The limit for `mention_doc` is 50 - The limit for `mention_user` is 100 **Data validation rules**: - Minimum length: `1` |
...
|     `style` | `text_style` | Yes | Text style. Supports Text, Heading1~9, Bullet, Ordered, Quote, Todo, and Task blocks |
...
|       `sequence` | `string` | No | Used to determine the number of an ordered list, either a number or 'auto' - Start a new list, numbering starts at 1, sequence='1' - Manually change the numbering, the numbering is set to a specific number, e.g. sequence='3' - Continue numbering, numbering is automatically consecutive, sequence='auto' - Some historical data and ordered block created through OpenAPI do not return this field **Example value**: ""auto"" |
|     `fields` | `int[]` | Yes | Fields that should be updated must specify at least one field. For example, to adjust Block alignment, set fields to [1]. **Example value**: Modified text style properties **Optional values are**:  - `1`: Modify Block Alignment - `2`: Modify todo block completion status - `3`: Folded state of text. Supports Heading1~9 block and the Text, Bullet, Ordered, Todo, and Task blocks with children - `4`: Code block language. Only supports Code block - `5`: Whether the code block wraps automatically. Only supports the Code block - `6`: Block background color - `7`: First line indentation level  |
|   `update_table_property` | `update_table_property_request` | No | Update table properties request. Only supports Table block |
...
|   `insert_table_row` | `insert_table_row_request` | No | Table insert new row request. Only supports Table block |
...
|   `insert_table_column` | `insert_table_column_request` | No | Table insert new column request. Only supports Table block |
...
|   `delete_table_rows` | `delete_table_rows_request` | No | Table batch delete rows request. Only supports Table block |
...
|   `delete_table_columns` | `delete_table_columns_request` | No | Table batch delete columns request. Only supports Table block |
...
|   `merge_table_cells` | `merge_table_cells_request` | No | Table merge cells request. Only supports Table block. Table cells need to meet one of the following conditions: - Completely contained within a previously merged area - Completely not within a previously merged area |
|     `row_start_index` | `int` | Yes | Row start index (interval closed left and right open) **Example value**: 0 **Data validation rules**: - Minimum value: `0` |
|     `row_end_index` | `int` | Yes | Row end index (interval closed left and right open) **Example value**: 1 **Data validation rules**: - Minimum value: `1` |
|     `column_start_index` | `int` | Yes | Column start index (interval closed left and right open) **Example value**: 0 **Data validation rules**: - Minimum value: `0` |
|     `column_end_index` | `int` | Yes | Column end index (interval closed left and right open) **Example value**: 1 **Data validation rules**: - Minimum value: `1` |
|   `unmerge_table_cells` | `unmerge_table_cells_request` | No | Table unmerge cells request. Only supports Table block |
...
|   `insert_grid_column` | `insert_grid_column_request` | No | Insert new column request. Only supports Grid block |
...
|   `delete_grid_column` | `delete_grid_column_request` | No | Delete column request. Only supports Grid block |
...
|   `update_grid_column_width_ratio` | `update_grid_column_width_ratio_request` | No | Update column width ratio request. Only supports Grid block |
...
|   `replace_image` | `replace_image_request` | No | Replace image request. You must ensure that you have uploaded an image before send this request |
...
|   `replace_file` | `replace_file_request` | No | Replace file request. You must ensure that you have uploaded a file before send this request |
|     `token` | `string` | Yes | file token **Example value**: "BoxbckbfvfcqEg22hAzN8Dh9gJd" |
|   `block_id` | `string` | No | Block unique identification **Example value**: "doxcnSS4ouQkQEouGSUkTg9NJPe" |
|   `update_text` | `update_text_request` | No | Update text element and style request |
|     `elements` | `text_element[]` | Yes | Updated list of text elements. In a single request: - Reminder limit: 30 - Mention_doc limit: 50 - Mention_user limit: 100 **Data validation rules**: - Minimum length: `1` |
|       `text_run` | `text_run` | No | Text. Supports Page, Text, Heading1~9, Bullet, Ordered, Code, Quote and Todo blocks |
|         `content` | `string` | Yes | Text content **Example value**: "text" |
|         `text_element_style` | `text_element_style` | No | Text local style |
|           `bold` | `boolean` | No | Bold **Example value**: true **Default value**: `false` |
...
|       `indentation_level` | `string` | No | First line indentation level **Example value**: "NoIndent" **Optional values are**:  - `NoIndent`: No indent - `OneLevelIndent`: One Level indent  |
|       `sequence` | `string` | No | Used to determine the number of an ordered list, either a number or 'auto' - Start a new list, numbering starts at 1, sequence='1' - Manually change the numbering, the numbering is set to a specific number, e.g. sequence='3' - Continue numbering, numbering is automatically consecutive, sequence='auto' - Some historical data and ordered block created through OpenAPI do not return this field **Example value**: ""auto"" |
|     `fields` | `int[]` | Yes | Fields in the text style that should be updated must specify at least one field. For example, to adjust Block alignment, set fields to [1]. **Example value**: [1] **Optional values are**:  - `1`: The alignment of blocks - `2`: Todo's status. Supports Todo and Task Blocks - `3`: Folded state of text. Supports Heading1~9 block and the Text, Bullet, Ordered, Todo, and Task blocks with children - `4`: Code block language. Only supports Code block - `5`: Modify the collapsed state of the code block - `6`: Block background color - `7`: First line indentation level  |
|   `update_task` | `update_task_request` | No | Update Task Block Request |
|     `task_id` | `string` | No | Task ID. This field only takes effect when the Task Block is updated for the first time. After the update is successful, the field will be ignored in subsequent requests. **Example value**: "ba5040f4-8116-4042-ab3c-254e5cfe3ce7" |
|     `folded` | `boolean` | No | Folded state, the folded state is not updated when the field is empty **Example value**: false | ### Request body example
...
| `data` | `\-` | \- |
|   `blocks` | `block[]` | Blocks for Batch Updates |
...
|   `revision_id` | `int` | The version number of the group announcement after the current update is successful |
|   `client_token` | `string` | The unique identifier of the operation, which is used in the update request to indicate idempotent for this update | ### Response body example
