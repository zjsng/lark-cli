...
url: "https://open.larksuite.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document-block-descendant/create"
method: "POST"
api_path: "https://open.larksuite.com/open-apis/docx/v1/documents/:document_id/blocks/:block_id/descendant"
service: "docx-v1"
resource: "document-block-descendant"
section: "Docs"
scopes:
  - "docx:document"
field_scopes:
  - "contact:user.employee_id:readonly"
updated: "1733381768000"
---

# Create nested blocks

Creates a batch of nested blocks (blocks with parent-child relationship) for a given block and inserts them at a specific location. The return value of this API is the rich text content of the newly created blocks.  To learn about the parent-child relationship between blocks, see Document overview-Basic concept. When the created child block contains GridColumn, TableCell, and Callout, it must contain at least one child block. That is, if the content is empty, an empty Text Block must be filled in as a child block.

> **Application frequency limit**: The maximum frequency of a single application call is 3 times per second, beyond which the HTTP status code  400  and the error code 99991400 will be returned.
> 
> **Document frequency limit**: The maximum number of concurrent edits per second for a single document is 3, beyond which the HTTP status code  429 will be returned. Editing operations include:
> - Create blocks
> - Create nested blocks
> - Delete blocks
> - Update a block
...
Before calling this API, please ensure that the current identity (tenant_access_token or user_access_token) already has the necessary permissions to read, edit, or perform other actions on cloud documents. Otherwise, the API will return an HTTP 403 or 400 status code. For more information, refer to How to Grant Document Permissions to Applications or Users.

## Request
| Facts |  |
| --- | --- |
| HTTP URL | https://open.larksuite.com/open-apis/docx/v1/documents/:document_id/blocks/:block_id/descendant |
| HTTP Method | POST |
...
| --- | --- | --- |
| `document_id` | `string` | The unique identification of the document. Click here to learn how to get `document_id` **Example value**: "Ew13dgs8BoZSetxd0CNbr0abcef" |
| `block_id` | `string` | The `block_id` of the parent block. To create child blocks for the document, you can fill in its `document_id` here. You can use the Obtain all blocks of a document to get the `block_id`. To learn about the parent-child relationship between blocks, see Document overview-Basic concept **Example value**: "Ew13dgs8BoZSetxd0CNbr0abcef" | ### Query parameters
| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
| `document_revision_id` | `int` | No | The document version to be queried. -1 indicates the latest version of the document. Once the document is created, the `document_revision_id` is 1. Make sure you have editing permission for the document. You can use the Obtain the basic information of a document interface to get the latest revision ID **Example value**: -1 **Default value**: `-1` **Data validation rules**: - Minimum value: `-1` |
| `client_token` | `string` | No | The unique identifier of the operation in UUIDv4 format, corresponding to the `client_token` returned by the API. It is used for idempotent update operations. When this value is null, a new request will be initiated; when this value is not null, idempotent updates will be performed **Example value**: "8aac2291-bc9e-4b12-a162-b3cf15bb06bd" |
...
| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
| `children_id` | `string[]` | Yes | List of temporary and customized IDs for the nested Blocks to be added. The ID can be any characters but must be unique in a single request. The server will generate the actual Block IDs and return them in the response body under the field "block_id_relations," indicating the mapping between temporary IDs and actual IDs **Example value**: ["child1", "child2"] **Data validation rules**: - Length range: `1` ～ `1000` |
| `index` | `int` | No | Specifies the placement of the newly created child blocks in the parent block. The starting value of the index is 0, indicating the first position in the child block list; The maximum value of the index is the number of child blocks of the parent block, representing the last position in the child block list. For example, if a parent block has 5 child blocks, their indexes are 0, 1, 2, 3, and 4, respectively. If you want to place a newly created block in the first position of the parent block, the index value should be 0; If you want to place the newly created block in the last position, the index value should be -1 **Example value**: 0 **Default value**: `-1` **Data validation rules**: - Minimum value: `-1` |
| `descendants` | `block[]` | Yes | List of nested blocks with parent-child relationship. To learn about the parent-child relationship between blocks, see Document overview-Basic concept **Data validation rules**: - Length range: `1` ～ `1000` |
|   `block_id` | `string` | No | The unique identification of a block **Example value**: "doxcnSS4ouQkQEouGSUkTg9NJPe" |
|   `children` | `string[]` | No | ID list of a block's children **Example value**: [doxcnO6UW6wAw2qIcYf4hZpFIth] |
...
|         `url` | `string` | Yes | Docs link (url_encode) **Example value**: "https%3A%2F%2Fexample.larksuite.com%2Fdocx%2Fdoxbc873Y7cXD153gXqb76abcef" |
|         `text_element_style` | `text_element_style` | No | Text element style |
|           `bold` | `boolean` | No | Bold **Example value**: true **Default value**: `false` |
|           `italic` | `boolean` | No | Italic **Example value**: true **Default value**: `false` |
|           `strikethrough` | `boolean` | No | Strikethrough **Example value**: true **Default value**: `false` |
...
|   `grid_column` | `grid_column` | No | Grid Column Block |
...
|   `table_cell` | `table_cell` | No | Table Cell Block |
|   `view` | `view` | No | View Block |
...
|     `url_type` | `string` | Yes | Link type **Example value**: "MessageLink" **Optional values are**:  - `MessageLink`: Message link - `Undefined`: Undefined link type  | ### Request example
```BASH
...
| `data` | `\-` | \- |
|   `children` | `block[]` | Block information for the added child |
...
|   `document_revision_id` | `int` | The version number of the document after the currently submitted Block is successfully created |
|   `client_token` | `string` | The unique identifier of the operation that is used in the update request to indicate idempotent for this update |
|   `block_id_relations` | `block_id_relation[]` | Mapping relationship between incoming temporary BlockID and real BlockID |
|     `temporary_block_id` | `string` | Temporary BlockID passed in by the user |
|     `block_id` | `string` | Real use of BlockID | ### Response body example
...
| 404 | 1770002 | not found | The `document_id` of the document does not exist. Please verify that the document has not been deleted or that the `document_id` is correct. Refer to Document overview for information on how to get the `document_id` of a document. |
...
| 400 | 1770022 | invalid page token | Confirm if the page_token in the query parameters is valid |
