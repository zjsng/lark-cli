---
title: "Add Wiki space members"
url: "https://open.larksuite.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/wiki-v2/space-member/create"
method: "POST"
api_path: "https://open.larksuite.com/open-apis/wiki/v2/spaces/:space_id/members"
service: "wiki-v2"
...
- Public Wiki space (visibility is public) is visible to all users of the tenant, so adding members is not supported, but admins can be added. Related error: 131101 public wiki space can't create members. 
 - Personal Wiki space (type is person) is a personally managed Wiki space, adding other admins (including applications/bots) is not supported. But members can be added. Related error: 131101 person wiki space can't create admin.
...
| --- | --- | --- |
| `space_id` | `string` | Wiki space id **Example value**: "1565676577122621" | ### Query parameters
| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
| `need_notification` | `boolean` | No | Whether to notify the other party after adding permissions **Example value**: True/fasle | ### Request body

| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
| `member_type` | `string` | Yes | "Openchat" - group id "userid" - user id "email" - email "opendepartmentMentid" - department id "openid" - openid for application "unionid" - unionid **Example value**: "userid" |
| `member_id` | `string` | Yes | User id **Example value**: "1565676577122621" |
| `member_role` | `string` | Yes | Role: "admin" -admin "member" -member **Example value**: "admin" | ### Request body example
...
| `data` | `\-` | \- |
|   `member` | `member` | Wiki Space Members |
|     `member_type` | `string` | "Openchat" - group id "userid" - user id "email" - email "opendepartmentMentid" - department id "openid" - openid for application "unionid" - unionid |
|     `member_id` | `string` | User id |
|     `member_role` | `string` | Role: "admin" -admin "member" -member | ### Response body example
...
| 400 | 131101 | Invalid operation | Invalid Wiki space operation. For example, public space prohibits adding members. Please refer to the specific interface error information and documentation description. |
