---
title: "Stream update text"
url: "https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/cardkit-v1/card-element/content"
method: "PUT"
api_path: "https://open.larksuite.com/open-apis/cardkit/v1/cards/:card_id/elements/:element_id/content"
service: "cardkit-v1"
...
scopes:
  - "cardkit:card:write"
updated: "1751018179000"
---

# Stream Updating Text

Stream Updating Text. Refer to the Streaming updates OpenAPI calling guide to understand the complete process of streaming updates for text.

## Output effect description

If the old text is a prefix substring of the incoming new text, the new text will continue to be output in typewriter effect at the end of the old text; if the old and new text have different prefixes, the full amount of text will be output directly onto the screen, without typewriter effects.
...
When calling this interface, make sure that the card's streaming update mode is enabled, i.e. streaming_mode is set to true.
...
| --- | --- | --- |
| `card_id` | `string` | Card entity ID. Get it by Create Card Entity **Example value**: "7355439197428236291" **Data validation rules**: - Length range: `1` ～ `20` characters |
| `element_id` | `string` | Component ID **Example value**: "elem_63529372" **Data validation rules**: - Length range: `1` ～ `64` characters | ### Request body
...
| `content` | `string` | Yes | Updated text content. **Note**: If the content contains code blocks, you need to remove the spaces before and after the code blocks, as they may cause the code rendering to fail. **Example value**: "updated text" **Data validation rules**: - Length range: `1` ～ `100000` characters |
...
### Error Codes
  ...
  The card's streaming update mode has been automatically closed due to a timeout. You can call Update Card Settings and set the `streaming_mode` field value to `true`.
      ...
      The card size exceeds the limit. Please keep the card size within 30KB.
      ...
      The card JSON data is empty. Please check the data.
    

  400
  300309
  Card streaming closed
  The streaming update mode is closed. You can call Update Card Settings and set the `streaming_mode` field value to `true`.
  

  400
  300310
  Only update text
  This interface only supports plain text components and rich text components.
      ...
      400
      300313
      Failed to update element properties
      Failed to update component properties. Please check the input parameters according to the error information returned by the interface.
      ...
      Service internal error. Please check whether the sequence parameter increments one by one and try again later. If the issue persists, contact [Technical Support](https://applink.larksuite.com/TLJpeNdW).
