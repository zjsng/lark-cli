...
url: "https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/reference/drive-v1/file-version/create"
method: "POST"
...
| --- | --- | --- |
| `file_token` | `string` | document token, how to get the document Token can refer to: How to get the token of docs resources **Example value**: "doxbcyvqZlSc9WlHvQMlSJwUrsb" | ### Query parameters
| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
...
| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
| `name` | `string` | No | title. This parameter is required, please ignore the "No" displayed in the required column on the left. The maximum length is 1024 Unicode code points. Generally, one English or Chinese character corresponds to one code point, but some special symbols may correspond to multiple code points. For example, the family combo "👨‍👩‍👧" this emoji corresponds to 5 code points. **Example value**: "new_name" |
| `obj_type` | `string` | No | document version type. This parameter is required, please ignore the "No" displayed in the required column on the left. **Example value**: "docx" **Optional values are**:  - `docx`: Upgraded Docs - `sheet`: Spreadsheet  | ### Request body example
...
|   `parent_type` | `string` | parent document token **Optional values are**:  - `docx`: Upgraded Docs - `sheet`: Spreadsheet  | ### Response body example
...
| --- | --- | --- | --- |
| 400 | 1068400 | Has no permission, please apply the file permission of reading or edition. | Please apply the file permission of reading or edition. |
