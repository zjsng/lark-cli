---
title: "Termination"
url: "https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/user/events/deleted"
service: "contact-v3"
resource: "user"
section: "Contacts"
scopes:
...
This event is triggered when an employee is terminated. The "Access Contacts as an app" is a scope of older version and is not recommended to be requested.{Usage Examples}(url=/api/tools/api_explore/api_explore_config?project=contact&version=v3&resource=user&event=deleted)
...
| Facts |  |
| --- | --- |
| Event type | contact.user.deleted_v3 |
...
| `event` | `\-` | \- |
|   `object` | `user_event` | Employee information |
...
|   `old_object` | `old_user_object` | User's information before the user is deleted |
|     `department_ids` | `string[]` | List of IDs of the user's departments **Required field scopes (Satisfy any)**: `contact:contact:readonly_as_app` `contact:user.department:readonly` `contact:contact:access_as_app` `contact:contact:readonly` |
...

{
    ...
    "event": {
        ...
        "old_object": {
            "department_ids": [
                "IDs of the employee's departments"
            ],
