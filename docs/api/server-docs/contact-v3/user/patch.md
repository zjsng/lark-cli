---
title: "Modify user information in part"
url: "https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/user/patch"
method: "PATCH"
...
updated: "1709623104000"
---

# Modify user information in part

This API is used to update a user's fields in Contacts. Parameters that are not specified will not be updated.

> Concurrent operations experience probabilistic failures due to transaction conflicts when freezing users. Try reducing the request rate or executing serially instead. When changing department_ids and is_frozen, the rate limit is limited to 1 QPS.
...
| Required scopes Enable any scope from the list | `contact:user.base` `contact:contact` |
| Required field scopes | > The response body of the API contains the following sensitive fields, and they will be returned only after corresponding scopes are added. If you do not need the fields, it is not recommended that you request the scopes. `contact:user.base:readonly` `contact:user.department:readonly` `contact:user.dotted_line_leader_info.read` `contact:user.employee:readonly` `contact:user.employee_number:read` `contact:user.gender:readonly` `contact:user.email:readonly` `contact:user.job_family:readonly` `contact:user.employee_id:readonly` `contact:user.job_level:readonly` `contact:user.phone:readonly` `contact:contact:access_as_app` `contact:contact:readonly` `contact:contact:readonly_as_app` | ### Request header
...
| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
| `name` | `string` | No | User name **Example value**: "John" **Data validation rules**: - Minimum length: `1` characters |
...
| `department_ids` | `string[]` | No | List of IDs of the user's departments. A user can belong to multiple departments. The ID value matches the department_id_type in the query parameter. For information about different IDs, see Department IDs. **Example value**: ["od-4e6ac4d14bcd5071a37a39de902c7141"] |
...
| `employee_type` | `int` | No | Employee type. Optional values are: - `1`: Regular - `2`: Intern - `3`: Outsourcing - `4`: Contractor - `5`: Consultant The int value of the custom employee type can also be read, and the name of the custom employee type for the tenant can be obtained through the API: Obtain workforce type **Example value**: 1 |
...
|     `text` | `string` | No | When the field type is `TEXT`, this parameter defines the field value and is required and limited to 100 characters; when the field type is `HREF`, this parameter defines the webpage title and is required. **Example value**: "DemoText" |
...
| `is_frozen` | `boolean` | No | Whether to freeze the user **Example value**: false |
| `job_level_id` | `string` | No | Job level ID **Example value**: "mga5oa8ayjlp9rb" |
| `job_family_id` | `string` | No | Job family ID **Example value**: "mga5oa8ayjlp9rb" |
| `subscription_ids` | `string[]` | No | The list of seat IDs assigned to users needs to be enabled with the "Assign User Seat" permission. The available seat ID of the tenant can be obtained through the interface below, see [Get Enterprise Seat Information] (/ssl: ttdoc/uAjLw4CM/ukTMukTMukTM/tenant-v2/tenant-product_assign_info/query) **Example value**: ["23213213213123123"] |
...
|     `name` | `string` | User name **Required field scopes (Satisfy any)**: `contact:user.base:readonly` `contact:contact:access_as_app` `contact:contact:readonly` `contact:contact:readonly_as_app` |
|     `en_name` | `string` | English name **Required field scopes (Satisfy any)**: `contact:user.base:readonly` `contact:contact:access_as_app` `contact:contact:readonly` `contact:contact:readonly_as_app` |
|     `nickname` | `string` | Alias **Required field scopes (Satisfy any)**: `contact:user.base:readonly` `contact:contact:access_as_app` `contact:contact:readonly` `contact:contact:readonly_as_app` |
...
|     `gender` | `int` | Gender **Optional values are**:  - `0`: Unknown - `1`: Male - `2`: Female - `3`: Other  **Required field scopes (Satisfy any)**: `contact:user.gender:readonly` `contact:contact:access_as_app` `contact:contact:readonly` `contact:contact:readonly_as_app` |
...
|     `avatar` | `avatar_info` | User's profile photo **Required field scopes (Satisfy any)**: `contact:user.base:readonly` `contact:contact:access_as_app` `contact:contact:readonly` `contact:contact:readonly_as_app` |
...
|     `status` | `user_status` | User status，enumeration types, including is_frozen, is_resigned, is_activated, is_exited User state transition see: User State Diagram **Required field scopes (Satisfy any)**: `contact:user.employee:readonly` `contact:contact:access_as_app` `contact:contact:readonly` `contact:contact:readonly_as_app` |
...
|     `department_ids` | `string[]` | List of IDs of the user's departments. A user can belong to multiple departments. The ID value matches the department_id_type in the query parameter. For information about different IDs, see Department IDs. **Required field scopes (Satisfy any)**: `contact:user.department:readonly` `contact:contact:access_as_app` `contact:contact:readonly` `contact:contact:readonly_as_app` |
|     `leader_user_id` | `string` | User ID of the user's direct manager. The ID value matches the user_id_type in the query parameter. For information about different IDs, see User-related IDs. **Required field scopes (Satisfy any)**: `contact:user.department:readonly` `contact:contact:access_as_app` `contact:contact:readonly` `contact:contact:readonly_as_app` |
|     `city` | `string` | work city **Required field scopes (Satisfy any)**: `contact:user.employee:readonly` `contact:contact:access_as_app` `contact:contact:readonly` `contact:contact:readonly_as_app` |
|     `country` | `string` | Country or region code. Refer to the list of country/region codes for details. **Required field scopes (Satisfy any)**: `contact:user.employee:readonly` `contact:contact:access_as_app` `contact:contact:readonly` `contact:contact:readonly_as_app` |
|     `work_station` | `string` | Seat ID **Required field scopes (Satisfy any)**: `contact:user.employee:readonly` `contact:contact:access_as_app` `contact:contact:readonly` `contact:contact:readonly_as_app` |
|     `join_time` | `int` | Entry time, in timestamp format, in seconds since January 1, 1970 **Required field scopes (Satisfy any)**: `contact:user.employee:readonly` `contact:contact:access_as_app` `contact:contact:readonly` `contact:contact:readonly_as_app` |
|     `is_tenant_manager` | `boolean` | Whether the user is a super administrator of the tenant. **Required field scopes (Satisfy any)**: `contact:user.employee:readonly` `contact:contact:access_as_app` `contact:contact:readonly` `contact:contact:readonly_as_app` |
...
|     `employee_type` | `int` | Employee type. Optional values are: - `1`: Regular - `2`: Intern - `3`: Outsourcing - `4`: Contractor - `5`: Consultant The int value of the custom employee type can also be read, and the name of the custom employee type for the tenant can be obtained through the API: Obtain workforce type **Required field scopes (Satisfy any)**: `contact:user.employee:readonly` `contact:contact:access_as_app` `contact:contact:readonly` `contact:contact:readonly_as_app` |
|     `orders` | `user_order[]` | User sorting information . Indicating the order of personnel in the organizational structure. Personnel may exist in multiple departments and have different orders. **Required field scopes (Satisfy any)**: `contact:user.department:readonly` `contact:contact:access_as_app` `contact:contact:readonly` `contact:contact:readonly_as_app` |
...
|     `custom_attrs` | `user_custom_attr[]` | Custom field. Make sure your company administrator has enabled "Allow to invoke by Open Platform's Contacts API" in Admin > Organization > Member Profile > Custom Fields > Global Settings; otherwise, this field will not take effect or be returned. more detail see FQAs **Required field scopes (Satisfy any)**: `contact:user.employee:readonly` `contact:contact:access_as_app` `contact:contact:readonly` `contact:contact:readonly_as_app` |
...
|     `enterprise_email` | `string` | Business email. Make sure that Lark Business Mail has been enabled in Admin. When create user, the detail of enterprise email see FQAs **Required field scopes (Satisfy any)**: `contact:user.employee:readonly` `contact:contact:access_as_app` `contact:contact:readonly` `contact:contact:readonly_as_app` |
|     `job_title` | `string` | Job title **Required field scopes (Satisfy any)**: `contact:user.employee:readonly` `contact:contact:access_as_app` `contact:contact:readonly` `contact:contact:readonly_as_app` |
...
| 409 | 41003 | user account conflict error | User's contact information involves two Lark accounts. Unable to add. The user can change the mobile number or email, or delete the account linked to the mobile number or email first, and create a new user or update the user. For how to delete an account, visit [Help Center](https://www.feishu.cn/hc/zh-CN/articles/360049067831). For more information about this error, please refer to the description in [General Error Codes]() |
...
| 403 | 41056 | no field authority error | No scope for this field. |
| 400 | 41057 | invalid employee type error | Invalid employee type. |
...
| 400 | 42008 | tenant id is invalid error | Check whether the tenant is valid. |
...
| 400 | 44003 | AvatarKey and description cannot be empty when updated resigned user | - |
...
| 400 | 44011 | exited user not allow to update | Exited user cannot be updated. |
...
| 400 | 44019 | feishu only support +86 mobile | Uncertified enterprises only support adding +86 mobile numbers. Please complete Lark verification before adding non +86 phone numbers. |
...
| 400 | 44035 | departmentID is invaild | Department ID is invalid |
| 400 | 44036 | freeze tenant founder is forbidden | Ban freeze tenant founder |
| 400 | 44038 | req set user geo not find in geo list | req set user geo not find in geo list |
| 400 | 44041 | Anonymized user info is not allowed to update | - |
...
| 400 | 44047 | license subscription id exceed limit | The seat id has exceeded the upper limit |
| 400 | 44048 | user license subscription id invalid | Please confirm to pass in the correct seat id |
| 400 | 44049 | license subscription update fail | Failed to update seat information, please try again |
| 403 | 44050 | not set subscription ids auth | Please open the "Assign User Seats" permission. |
