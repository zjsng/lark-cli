---
title: "Query the calendar list"
url: "https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/reference/calendar-v4/calendar/list"
method: "GET"
...
Call this interface to page through and query the calendar list of the current identity (application or user).
...
> - When calling this interface, you first need to use page_token to paginate and query the existing calendar list, and then use sync_token to incrementally synchronize the changes in the calendar data.
...
| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
| `page_size` | `int` | No | The maximum number of calendars returned in a single request. **Example value**: `50` **Default value**: `500` **Data validation rules**: - Value range: `50` ～ `1000` |
| `page_token` | `string` | No | Page identifier. It is not filled in the first request, indicating traversal from the beginning; when there will be more groups, the new page_token will be returned at the same time, and the next traversal can use the page_token to get more groups **Example value**: ListCalendarsPageToken_xxx |
| `sync_token` | `string` | No | Incremental synchronization mark, no need to fill in for the first request. When the page query ends (the returned value of page_token is empty), the interface will return the sync_token field. The next call can use this sync_token to incrementally obtain calendar change data. **Example value**: ListCalendarsSyncToken_xxx | ## Response
...
|   `sync_token` | `string` | Incremental synchronization mark. When has_more is false, it will synchronously return a new sync_token, and the next request needs to carry the sync_token to incrementally obtain the calendar change data. **Note**: The returned sync_token is valid within 90 days. |
|   `calendar_list` | `calendar[]` | List of calendar data loaded by pages. |
...
| 400 | 190009 | sync token cannot be used with other request restrictions | sync_token cannot be used with other conflicting parameters. You need to refer to the sync_token parameter restrictions and modify it to the correct parameter configuration. |
