---
title: "Query the binding status of the Exchange account"
url: "https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/reference/calendar-v4/exchange_binding/get"
method: "GET"
...
Call this interface to obtain the binding status of the Exchange account, including the synchronization status of the Exchange calendar.
...
| Required scopes Enable any scope from the list | `calendar:calendar:readonly` `calendar:exchange.bindings:read` |
...
| --- | --- | --- |
| `exchange_binding_id` | `string` | The unique identification ID bound to Exchange. When calling Bind Exchange Account to Lark Account, the exchange_binding_id can be obtained from the returned result. **Example value**: "ZW1haWxfYWRtaW5fZXhhbXBsZUBvdXRsb29rLmNvbSBlbWFpbF9hY2NvdW50X2V4YW1wbGVAb3V0bG9vay5jb20=" | ### Query parameters
| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
...
|   `user_id` | `string` | User ID, that is, the Lark account ID bound to the Exchange account. For more information, see User-related IDs. |
...
|   `exchange_binding_id` | `string` | The unique identification ID bound to Exchange, which can be used for subsequent queries or unbinding Exchange. | ### Response body example
