...
url: "https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/reference/vc-v1/room/events/updated"
service: "vc-v1"
resource: "room"
...
This event is triggered when a meeting room is updated.{Usage Examples}(url=/api/tools/api_explore/api_explore_config?project=vc&version=v1&resource=room&event=updated)

## Event
| Facts |  |
| --- | --- |
| Event type | vc.room.updated_v1 |
...
| `event` | `\-` | \- |
|   `room` | `room_event` | Meeting room details |
...
|     `room_status` | `room_status_event` | Room status |
...
|       `contact_ids` | `user_id[]` | Contact user ID |
|         `union_id` | `string` | Union id of user |
|         `user_id` | `string` | User id of user |
|         `open_id` | `string` | Open id of user |
|       `disable_notice` | `boolean` | Notice content sent to users when room disabled |
|       `resume_notice` | `boolean` | Notice content sent to users when room resumed |
|     `device` | `device[]` | device information |
|       `name` | `string` | device name | ### Event body example
