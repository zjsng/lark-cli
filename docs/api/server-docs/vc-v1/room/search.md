...
url: "https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/reference/vc-v1/room/search"
method: "POST"
api_path: "https://open.larksuite.com/open-apis/vc/v1/rooms/search"
service: "vc-v1"
resource: "room"
...
| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
| `custom_room_ids` | `string[]` | No | Custom room ID **Example value**: ["10001"] |
| `keyword` | `string` | No | Search keyword(Used when custom room ID is empty) **Example value**: "TestRoom" |
| `room_level_id` | `string` | No | Search room at this room level(Used when custom room ID is empty) **Example value**: "omb_4ad1a2c7a2fbc5fc9570f38456931293" |
| `search_level_name` | `boolean` | No | Search room that include room level name(Used when custom room ID is empty) **Example value**: true |
| `page_size` | `int` | No | Page size(Used when custom room ID is empty) **Example value**: 10 **Default value**: `10` **Data validation rules**: - Value range: `1` ～ `100` |
| `page_token` | `string` | No | Page identifier. It is not filled in the first request, indicating traversal from the beginning; when there will be more groups, the new page_token will be returned at the same time, and the next traversal can use the page_token to get more groups **Example value**: "0" | ### Request body example
