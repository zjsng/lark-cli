---
title: "Start of corporate meeting"
url: "https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/reference/vc-v1/meeting/events/all_meeting_started"
service: "vc-v1"
resource: "meeting"
...
Occurs at the start of a meeting and includes all meeting start events within the enterprise.{Usage Examples}(url=/api/tools/api_explore/api_explore_config?project=vc&version=v1&resource=meeting&event=all_meeting_started)

## Event
| Facts |  |
| --- | --- |
| Event type | vc.meeting.all_meeting_started_v1 |
| Supported app types | custom,isv |
| Required scopes | `vc:meeting.all_meeting:readonly` |
...
| `event` | `\-` | \- |
|   `meeting` | `meeting_event_meeting` | Meeting data |
...
|     `meeting_source` | `int` | Meeting Creation Source **Optional values are**:  - `1`: Schedule Meeting - `2`: Instant meeting - `3`: Interview meeting - `4`: Open platform meeting - `100`: Other meeting types  |
|     `start_time` | `string` | Meeting start time (Unix time in sec) |
|     `end_time` | `string` | Meeting end time (Unix time in sec) |
|     `host_user` | `meeting_event_user` | Meeting host |
|       `id` | `user_id` | User ID |
...
|       `user_role` | `int` | User's role in a meeting **Optional values are**:  - `1`: Participant - `2`: Host - `3`: Co-host  |
...
|     `owner` | `meeting_event_user` | Meeting owner |
|       `id` | `user_id` | User ID |
|         `union_id` | `string` | User's union_id |
|         `user_id` | `string` | user's user id **Required field scopes**: `contact:user.employee_id:readonly` |
|         `open_id` | `string` | User's open_id |
|       `user_role` | `int` | User's role in a meeting **Optional values are**:  - `1`: Participant - `2`: Host - `3`: Co-host  |
|       `user_type` | `int` | User type **Optional values are**:  - `1`: Lark user - `2`: Rooms user - `3`: Docs user - `4`: neo Lark Meetings user - `5`: neo Lark Meetings guest - `6`: PSTN user - `7`: SIP user  |
|     `calendar_event_id` | `string` | Unique identifier of the calendar event |
|   `operator` | `meeting_event_user` | Event operator |
|     `id` | `user_id` | User ID |
|       `union_id` | `string` | User's union_id |
|       `user_id` | `string` | user's user id **Required field scopes**: `contact:user.employee_id:readonly` |
|       `open_id` | `string` | User's open_id |
|     `user_role` | `int` | User's role in a meeting **Optional values are**:  - `1`: Participant - `2`: Host - `3`: Co-host  |
|     `user_type` | `int` | User type **Optional values are**:  - `1`: Lark user - `2`: Rooms user - `3`: Docs user - `4`: neo Lark Meetings user - `5`: neo Lark Meetings guest - `6`: PSTN user - `7`: SIP user  | ### Event body example
