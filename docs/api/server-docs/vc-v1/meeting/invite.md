...
url: "https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/reference/vc-v1/meeting/invite"
method: "PATCH"
api_path: "https://open.larksuite.com/open-apis/vc/v1/meetings/:meeting_id/invite"
service: "vc-v1"
resource: "meeting"
...
> Operators must have relevant permission scope to invite participants. If the operator is a user, they must be the current host of the meeting. If the meeting is locked, or the number of participants has reached the upper limit, the invitation will fail.
...
| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
| `invitees` | `meeting_user[]` | Yes | List of invited users [Up to 10 people can be invited at one time] |
...
| `data` | `\-` | \- |
|   `invite_results` | `meeting_invite_status[]` | Invitation result |
|     `id` | `string` | User ID |
...
|     `status` | `int` | Invitation result **Optional values are**:  - `1`: Invitation succeeded - `2`: Invitation failed  | ### Response body example
...
| 400 | 122006 | participant count reach limit | The number of participants has reached the upper limit. |
| 400 | 122007 | callee is busy | The callee is busy. |
| 400 | 122008 | meeting is locked | Meeting is locked. |
