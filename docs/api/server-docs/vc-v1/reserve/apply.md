...
url: "https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/reference/vc-v1/reserve/apply"
method: "POST"
api_path: "https://open.larksuite.com/open-apis/vc/v1/reserves/apply"
service: "vc-v1"
resource: "reserve"
...
updated: "1694053218000"
---

# Schedule a meeting

Creates a meeting scheduling.

> Support reservation for meetings within the last 30 days (the expiration time is no more than 30 days from the current time). After the appointment expires, the meeting number will be released. If you need to continue to use it, you can renew it through the "Update Appointment" interface; when booking a meeting, you can Configure the permissions of the participants in the meeting to achieve the purpose of controlling the meeting.
> 
> Meetings booked in this way 「will not」 generate a schedule, and 「will not」display in the calendar.
...
| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
| `end_time` | `string` | No | Expiration time (Unix time in seconds), which is required for multi-person meetings. **Example value**: "1608888867" |
| `owner_id` | `string` | No | Assign meeting owner (Only take effect when using tenant_access_token. Will not take effect when using user_access_token). Must assign user with the same tenant. **Example value**: "ou_3ec3f6a28a0d08c45d895276e8e5e19b" |
| `meeting_settings` | `reserve_meeting_setting` | Yes | Meeting settings |
...
|   `meeting_initial_type` | `int` | No | Initial type of meeting **Example value**: 1 **Optional values are**:  - `1`: Multi-person meeting - `2`: 1v1 calling(Only support PSTN)  |
...
"end_time":"1608883322"},
"reserve_correction_check_info":{"invalid_host_id_list":["ou_3ec3f6a28a0d08c45d895276e8e5e19b"]}}}
